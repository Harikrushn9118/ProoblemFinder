...
const SolveProblem = () => {
    ...
    const fetchProblem = async () => {
        ...
        try {
            ...
            setError('Error in fetching problems');
        }
        setLoading(false);
    };

    const fetchingSolveProb = async () => {
        if (!username) {
            return;
        }
        try {
            ...
            setError("Error fetching Solved Problem");
        }
    };

    const toggleBookmark = (probl) => {
        const problId = `${probl.contestId}-${probl.index}`;
        if (bookmark.some(p => `${p.contestId}-${p.index}` === problId)) {
            setBookmark(bookmark.filter(p => `${p.contestId}-${p.index}` !== problId));
        } else {
            setBookmark([...bookmark, probl]);
        }
    };
        ...
        <div className="solve-problem-container">
            <div className="filters-section">
                <div className="username-section">
                    ...
                    />
                    <button onClick={fetchingSolveProb} className="Trackbutton">Track Progress</button>
                </div>
                ...
                <button onClick={fetchProblem} disabled={loading || !rating}>
            ...
            <div className="problems-grid">
                {problems.map(problem => {
                    ...
                    const isBookmarked = bookmark.some(p => `${p.contestId}-${p.index}` === problemId);
